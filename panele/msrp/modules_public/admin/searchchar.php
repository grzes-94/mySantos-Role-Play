<?php
class public_msrp_admin_searchchar extends ipsCommand
{
    ...
    if(!empty($this->request['nick']))
    {
      ...
      if(!preg_match('/^[A-Za-z_]+$/',$nick))
        $this->registry->output->showError('Wprowadzona nazwa postaci zaiwera niedozwolone znaki.',0);
      $query = $this->DB->query("SELECT * FROM `srv_characters` WHERE `nickname` LIKE '%$nick%'");
      while($row = $query->fetch_assoc())
      {
        $row['nickname']=$functions->GetCharacterName($row['player_uid'],true);
	...
	$template = $this->registry->output->getTemplate('msrp')->msrp_admin_searchchar($chars);
